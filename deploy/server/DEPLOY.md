...
### Порты 80 и 443: не нужны «свободные порты» под каждый сайт
...
# или
sudo lsof -iTCP -sTCP:LISTEN -P | grep nginx
```

Должен быть **один** master-процесс nginx на **:80** и **:443**. Если порт занят не nginx — смотрите, что за служба (`apache2`, другой прокси) и не конфликтует ли.

На сервере с **многими** сайтами в `sites-enabled/` это нормально: `ukudarov.pro`, `elearning.aqlant.com` и т.д. **не требуют** разных портов. Важно лишь, чтобы не было **двух** блоков с одинаковым `server_name` на одном `listen` (иначе предупреждение `conflicting server name`).

Чтобы не показывалась заглушка **«Welcome to nginx!»**, отключите дефолтный сайт (если он ещё включён):
